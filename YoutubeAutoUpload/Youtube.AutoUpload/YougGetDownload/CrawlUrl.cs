...
namespace Youtube.AutoUpload.YougGetDownload
{
    public class CrawlUrl
    {
        ...
        public List<ConfigureFileModel> ExtractUrlFromHtml(string url)
        {
           
            var models = new List<ConfigureFileModel>();

            HtmlDocument htmlDoc = new HtmlDocument();
            htmlDoc.LoadHtml(RequestAction(new RequestOptions
            {
                Uri = new Uri(url),
                Method = "GET"
            }));
            HtmlNodeCollection liNodes = htmlDoc.DocumentNode.SelectSingleNode("//div[@class='contentView']").SelectNodes("a");
            if (liNodes != null && liNodes.Count > 0)
            {
                for (int i = 0; i < liNodes.Count; i++)
                {
                    string title = liNodes[i].InnerText.Trim();
                    string href = liNodes[i].GetAttributeValue("href", "").Trim();
                    Console.WriteLine("标题：" + title + ",链接：" + href);
                    var model = new ConfigureFileModel
                    {
                        Description = "",
                        Title = title,
                        DownloadFilePath = "",
